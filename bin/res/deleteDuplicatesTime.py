...
def DeleteDuplicates(d):
    ...
    try:
        files = os.listdir(d)
        files = [(separator.join(f.split(separator)[:-2]), separator.join(f.split(separator)[-2:])) if
                  len(f.split(separator)) > 2 else (f,'') for f in files]
        mappedFiles = dict([(key, []) for (key, value) in files])
        for (key, value) in files:
	    if len(value) > 0:
                mappedFiles[key] = sorted(mappedFiles[key] + [value])
